...
$V(j,k)=\sum_{i=1}^{N} \frac{e^{w_{k}^{T}x_{i}+b_{k}}}{\sum_{k'}^{N} e^{w_{k}^{T}x_{i}+b_{k}}} (x_{i}(j)-c_{k}(j))$, where {$w_{k}$}, {$b_{k}$} and {$c_{k}$} are sets of trainable parameters for each cluster $k$.
